-- Atomic demo trade open: balance debit + trade creation in one transaction.
...
BEGIN
  ...
  END IF;
  SELECT * INTO v_account FROM public.demo_accounts WHERE user_id = p_user_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Demo account not found'; END IF;
  IF v_account.balance < p_investment THEN RAISE EXCEPTION 'Insufficient demo balance'; END IF;
  ...
  INSERT INTO public.demo_trades (
    ...
    stop_loss, take_profit, signal_id, signal_type, ai_confidence, status, opened_at, updated_at
  ) VALUES (
    p_user_id, p_symbol, p_pair, p_coin_name, p_buy_price, p_quantity, p_investment,
    p_stop_loss, p_take_profit, p_signal_id, p_signal_type, p_ai_confidence, 'open', now(), now()
  )
  RETURNING *;
END;
$$;

REVOKE ALL ON FUNCTION public.open_demo_trade_atomic(uuid,text,text,text,numeric,numeric,numeric,numeric,numeric,text,text,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.open_demo_trade_atomic(uuid,text,text,text,numeric,numeric,numeric,numeric,numeric,text,text,integer) TO authenticated;
